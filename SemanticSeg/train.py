...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.weight:
        if os.path.isfile(args.weight):
            ...
            model.backbone.load_pretrained_model(
                args.weight,
                skip_first_conv=args.get("skip_first_conv", True),
                verbose=main_process(),
            )
            if main_process():
    ...
    if args.scheduler == "MultiStepWithWarmup":
        ...
        if main_process():
            logger.info(
                "scheduler: CosineWithWarmup. scheduler_update: {}".format(
                    args.scheduler_update
                )
            )
        iter_per_epoch = len(train_loader)
    ...
    for i, (coord, feat, target, offset, target_pts, inverse_map) in enumerate(
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info(
                "Test Points: [{}/{}] "
                "Accuracy {accuracy:.4f}.".format(
                    i + 1, len(val_loader), accuracy=accuracy
                )
            )
