...
class Scannetv2_Point(Scannetv2):
    def __init__(self, split='train', data_root='trainval', voxel_size=0.04, voxel_max=None, transform=None, shuffle_index=False, loop=1, vote_num=1):
        super().__init__(split, data_root, voxel_size, voxel_max, transform, shuffle_index, loop)
    ...
    def __getitem__(self, idx):
        ...
        coord, feat, label, label_pts, inverse_map = data_prepare_scannet_point(coord, feat, label, self.split, self.voxel_size, self.voxel_max, self.transform, self.shuffle_index)
...
class Scannetv2_Normal(Dataset):
    def __init__(self, split='train', data_root='trainval', voxel_size=0.04, voxel_max=None, transform=None, shuffle_index=False, loop=1, vote_num=1):
        ...
        if split == "train" or split == 'val':
            self.data_list = glob.glob(os.path.join(data_root, split, "*_vh_clean_2.ply"))
        elif split == 'trainval':
            self.data_list = glob.glob(os.path.join(data_root, "train", "*_vh_clean_2.ply")) + glob.glob(os.path.join(data_root, "val", "*_vh_clean_2.ply"))
        elif split == 'trainval_v2':
            train_data_list = glob.glob(os.path.join(data_root, 'train', "*_vh_clean_2.ply"))
            val_data_list = glob.glob(os.path.join(data_root, 'val', "*_vh_clean_2.ply"))
            train_data_list.sort()
            val_data_list.sort()
            self.data_list = train_data_list+val_data_list[:-100]
