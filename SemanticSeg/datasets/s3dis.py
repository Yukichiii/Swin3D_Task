...
class S3DIS(Dataset):
    def __init__(self, split='train', data_root='trainval', test_area=5, voxel_size=0.04, voxel_max=None, transform=None, shuffle_index=False, loop=1,   vote_num=1):
        super().__init__()
...
class S3DIS_Point(S3DIS):
    def __getitem__(self, idx):
        ...
        coord, feat, label = data[:, 0:3], data[:, 3:6], data[:, 6]
        feat = feat / 127.5 - 1
        coord, feat, label, label_pts, inverse_map = data_prepare_s3dis_point(coord, feat, label, self.split, self.voxel_size, self.voxel_max, self.transform, self.shuffle_index)
