...
def get_parser():
    ...
    parser.add_argument('--save_output', action='store_true', default=False, help='whether store output')
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.data_name == 'scannetv2':
        val_data = Scannetv2_Point(split=val_split, data_root=args.data_root, voxel_size=args.voxel_size, voxel_max=800000, transform=val_transform,  vote_num=vote_num)
    elif args.data_name == 'scannetv2_normal':
        val_data = Scannetv2_Normal_Point(split=val_split, data_root=args.data_root, voxel_size=args.voxel_size, voxel_max=800000, transform=val_transform,  vote_num=vote_num)
    elif args.data_name == 's3dis':
        val_data = S3DIS_Point(split=val_split, data_root=args.data_root, test_area=args.test_area, voxel_size=args.voxel_size, voxel_max=800000, transform=val_transform,  vote_num=vote_num)

    val_loader = torch.utils.data.DataLoader(val_data, batch_size=1, shuffle=False, num_workers=args.workers, \
            pin_memory=True, sampler=None, collate_fn=collate_fn_pts)
...
def validate(val_loader, model, vote_num=12):
    ...
    if vote_num>12:
        save_path = args.weight.split('.')[0] + "/" + args.get("val_split", "val") + f"_vote{vote_num}"
    else:
    ...
    for i in range(val_num):
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Test Points: [{}/{}] '
                        ...
                        'Accuracy {accuracy:.4f}.'.format(i + 1, val_num,
    ...
    if main_process():
        ...
        for i in range(args.classes):
            logger.info('Class_{:02d} Result: iou/accuracy {:.4f}/{:.4f}.'.format(i, iou_class[i], accuracy_class[i]))
