...
def collate_fn_pts(batch):
    ...
    for item,inverse in zip(coord, inverse_map):
        ...
        count += item.shape[0]
        offset.append(count)
    return torch.cat(coord), torch.cat(feat), torch.cat(label), torch.IntTensor(offset), torch.cat(label_pts), torch.cat(inverse_list)
...
def data_prepare_s3dis_point(coord, feat, label, split='train', voxel_size=0.04, voxel_max=None, transform=None, shuffle_index=False):
    if transform:
        # coord, feat, label = transform(coord, feat, label)
        color = feat[:, 0:3]
        coord, color = transform(coord, color)
...
def data_prepare_scannet_point(coord, feat, label, split='train', voxel_size=0.04, voxel_max=None, transform=None, shuffle_index=False):
    if transform:
        # coord, feat, label = transform(coord, feat, label)
        color = feat[:, 0:3]
        normal = feat[:, 3:6]
