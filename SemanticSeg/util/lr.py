...
class MultiStepWithWarmup(LambdaStepLR):
    def __init__(
        ...
        super(MultiStepWithWarmup, self).__init__(
            optimizer, multi_step_with_warmup, last_step
        )
...
def initialize_scheduler(
    optimizer, config, last_epoch=-1, scheduler_epoch=True, logger=None
):
    # scheduler_epoch: the step_size are given in epoch num
    last_step = (
        -1 if last_epoch < 0 else config.iter_per_epoch_train * (last_epoch + 1) - 1
    )
    if scheduler_epoch:
    ...
    if config.scheduler == "StepLR":
        ...
        return ExpLR(
            optimizer,
            step_size=config.exp_step_size,
        ...
        return OneCycleLR(
            optimizer,
            max_lr=config.oc_max_lr,
            total_steps=config.max_iter,
            pct_start=config.oc_pct_start,
            anneal_strategy=config.oc_anneal_strategy,
            div_factor=config.oc_div_factor,
            final_div_factor=config.oc_final_div_factor,
            last_epoch=last_step,
        )
