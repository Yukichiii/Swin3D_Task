...
def build_optimizer(model, optim_cfg, fix_encoder=False, relax_table=False):
    transformer_lr_scale = optim_cfg.get("transformer_lr_scale", 1.0)
    if transformer_lr_scale != 1.0:
        print(f"Use transformer_lr_scale:{transformer_lr_scale} for blocks")
        params = [
            {"params": [p for n, p in model.named_parameters() if "blocks" not in n and p.requires_grad]},
            {
                ...
                "lr": optim_cfg.LR * transformer_lr_scale,
            },
        ]
    else:
