...
from .sampler import VoxelStridePointSampler, DistancePointSampler
...
class DataAugmentor(object):
    ...
    def voxel_sampler(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.voxel_sampler, config=config)

        if not hasattr(self, "voxelizer"):
            self.voxelizer = VoxelStridePointSampler(config["voxel_stride"])
        return self.voxelizer(data_dict)

    def distance_sampler(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.distance_sampler, config=config)

        if not hasattr(self, "dist_sampler"):
            self.dist_sampler = DistancePointSampler(num_points=config["num_points"], 
                                                    box_threshold=config["box_threshold"],
                                                    valid_sample=config["valid_sample"],
                                                    num_classes=config["num_classes"])
        return self.dist_sampler(data_dict)
