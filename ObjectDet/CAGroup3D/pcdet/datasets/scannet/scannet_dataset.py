...
class ScannetDataset(DatasetTemplate):
    ...
    def generate_prediction_dicts(batch_dict, pred_dicts, class_names, output_path=None):
        ...
        for index, box_dict in enumerate(pred_dicts):
            ...
            if output_path is not None:
                pred_boxes = single_pred_dict['boxes_3d']
                pred_scores = single_pred_dict['scores_3d']
                pred_labels = single_pred_dict['labels_3d']
                outname = output_path / ("%s.npz"%frame_id)
                print("save to: ", outname)
                np.savez(outname, pred_boxes=pred_boxes, pred_labels=pred_labels, pred_scores=pred_scores)
    ...
    def prepare_data(self, data_dict):
        ...
        if self.training:
            ...
            gt_classes = np.array([self.class_names.index(n) if n in self.class_names else -1 for n in data_dict['gt_names']], dtype=np.int32)

            data_dict = self.data_augmentor_train.forward(
                data_dict={
                    **data_dict,
                    'gt_boxes_label': gt_classes,
...
class ScannetNormalDataset(ScannetDataset):
    def get_lidar(self, idx):
        lidar_file = self.root_path / 'points_normal' / ('%s.bin' % idx)
        assert lidar_file.exists()
        return np.fromfile(str(lidar_file), dtype=np.float32).reshape(-1, 9)
